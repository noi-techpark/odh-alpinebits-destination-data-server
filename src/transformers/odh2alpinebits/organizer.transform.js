...
function transformOrganizer(originalObject, included, request) {
  ...
  if (!sourceOrganizer) return null;

  let organizer = templates.createObject("Agent", request.apiVersion);
  ...
  organizerAddress.zipcode = utils.safeGetOne(
    [
      ["zipcode", "ita"],
      ["zipcode", "eng"],
      ["zipcode", "deu"],
  ...
  for (languageEntry of utils.languageMapping) {
    ...
    if (sourceEventOrganizer) {
      ...
      const orgId =
        sourceOrganizerID ||
        utils.safeGetString(["Tax"], sourceEventOrganizer) ||
        utils.safeGetString(["Vat"], sourceEventOrganizer) ||
        email;
  ...
  if (!organizer.id) organizer.id = sourceEvent.Id + "+sourceOrganizer";
  ...
  if (!newContact.email)
    newContact.email = utils.safeGetOne(
      [
        ["de", "Email"],
        ["it", "Email"],
        ["en", "Email"],
  ...
  if (!newContact.telephone)
    newContact.telephone = utils.safeGetOne(
      [
        ["de", "Phonenumber"],
        ["it", "Phonenumber"],
        ["en", "Phonenumber"],
  ...
  if (!newContact.address.city || !newContact.address.country) newContact.address = null;

  if (!newContact.email && !newContact.telephone && !newContact.address) attributes.contactPoints = null;
  ...
  if (!request.apiVersion || request.apiVersion === "1.0") {
    attributes.categories =
      !inferredType.organization && inferredType.person ? ["alpinebits/person"] : ["alpinebits/organization"];
  } else if (!request.apiVersion || request.apiVersion === "1.0") {
    const category =
      !inferredType.organization && inferredType.person ? "alpinebits:person" : "alpinebits:organization";
    const categoryReference = { type: "categories", id: category };
    utils.addRelationshipToMany(organizer.relationships, "categories", categoryReference, organizer.links.self);
  }
