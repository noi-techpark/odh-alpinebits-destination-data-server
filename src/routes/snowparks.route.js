...
module.exports = function (app) {
  ...
  app.get("/2.0/snowparks", function (req, res) {
    try {
      connector
        .getSnowparks(parseCollectionRequest(req))
        .then((data) => res.json(data))
  ...
  app.get("/2.0/snowparks/:id", function (req, res) {
    try {
      connector
        .getSnowparkById(parseResourceRequest(req))
        .then((data) => res.json(data))
  ...
  app.get("/2.0/snowparks/:id/categories", function (req, res) {
    errors.handleNotImplemented(req, res);
  });

  app.get("/2.0/snowparks/:id/connections", function (req, res) {
    errors.handleNotImplemented(req, res);
  });

  app.get("/2.0/snowparks/:id/features", function (req, res) {
    errors.handleNotImplemented(req, res);
  });

  app.get("/2.0/snowparks/:id/multimediaDescriptions", function (req, res) {
    try {
      connector
        .getSnowparkMedia(parseResourceRequest(req))
        .then((data) => res.json(data))
