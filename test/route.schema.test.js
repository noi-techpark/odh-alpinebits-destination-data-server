...
module.exports.basicSchemaTests = (opts) => {
  ...
  describe(`Individual resource messages should validate against AlpineBits and JSON:API schemas on routes /${opts.route}/:id`, () => {
    ...
    beforeAll( () => {
      return utils.axiosInstance.get(`/1.0/${opts.route}?page[size]=10&page[number]=1`)
        .then( response => {
          resourceArray = response.data.data;
        })
    })

    test(`Checking the first 10 resources on /1.0/${opts.route}/`, () => {
      let requests = [];
      
      for (let i = 0; i < resourceArray.length; i++) {
        let id = resourceArray[i].id;
        let request = utils.axiosInstance.get(`/1.0/${opts.route}/${id}`);
      ...
      return Promise.all(requests)
        .then( resArray => {
          resArray.forEach( res => {
            ...
            if(!isValid)
              console.log("Invalid AlpineBits schema:", res.data.id);
            
            expect(isValid).toBe(true);
            
            if(!isValid)
              console.log("Invalid JSON:API schema:", res.data.id);
