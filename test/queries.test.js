...
module.exports.basicQueriesTest = (opts) => {
  describe(`Basic queries tests for /1.0/${opts.route}`, () => {
    opts.queries.forEach((queryItem) => {
      const { query, expectStatus, description } = queryItem;
      const path = `/1.0/${opts.route}?${query}`
      
      test(`${path}${description ? ' - ' + description : ''}`, () => 
        utils.axiosInstance.get(path).then((response) => {
...
let sample = {
  queries: [
    {
      route: '/1.0/events/FD7DC58E68A9447FB987460138FB3C85',
